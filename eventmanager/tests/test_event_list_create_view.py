...
class EventListCreateTest(BaseTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_get_event_list(self):
        ...
        self.assertEqual(
            expected_data, response.data, "Response data did not match expected data."
        )
    ...
    def test_post_events(self):
        ...
        response = self.client.post(
            "/api/events/", {"name": "click", "description": "clicked"}, format="json"
        )
        ...
        response = self.client.post(
            "/api/events/", {"one": "click", "two": "clicked"}, format="json"
        )
    ...
    def test_search_event(self):
        ...
        Event.objects.create(
            name="Test Event", description="This is a test event.", user=self.user1
        )

        # Use the search feature
        response = self.client.get(
            "/api/events/", {"search": "Test Event"}, format="json"
        )
